...
export class FeedController {
  ...
  @Get('events')
  async getFeedEvents(@Query('limit') limit: number, @Query('from') from: string, @Query('to') to: string): Promise<FeedEvent[]> {
    return this.feedEventService.find(limit, from, to);
  }

  @Patch()
  async updateFeedEvent(@Body() patchFeedDTO: PatchFeedDTO): Promise<FeedResponse> {
