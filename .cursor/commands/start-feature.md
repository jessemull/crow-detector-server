# Start feature branch (Crow Detector Server)

Branch name: **$ARGUMENTS**

If `$ARGUMENTS` is empty, stop and ask for a feature branch name (Conventional Commit scope style, e.g. `feat/feed-cooldown` or `chore/governance-docs`).
...
Ensure dependencies (and husky) are installed:

```bash
npm install
cd lambda && npm install && cd ..
test -f .husky/pre-commit && echo OK
```
